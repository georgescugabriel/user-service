...
class Handler extends ExceptionHandler
{
    ...
    {
        $response = $this->extractStatusCode($exception);
        $statusCode = $response['status'];
        $error['error_code'] = $statusCode;
        $error['error'] = Response::$statusTexts[$statusCode];
        if (env('APP_DEBUG')) {
            $error['debug']['message'] = $exception->getMessage();
            $error['debug']['file'] = $exception->getFile().':'.$exception->getLine();
            $error['debug']['trace'] = explode("\n", $exception->getTraceAsString());
        }
     ...
     * @return array
     */
    protected function extractStatusCode (\Throwable $exception) : array
    {
